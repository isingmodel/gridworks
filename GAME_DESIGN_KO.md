...
| 건설 | 자유 배치, 거리·강 횡단에 따라 비용과 기간 산정; 철탑 수는 표시용 파생값 |
| 물리 | 그래프 기반 전력조류, 손실, 선로 열, 설비 용량, 섬 분리 |
| 승리 | 의무 수요를 안전 기준 이상으로 공급하면서 3장 캠페인 총비용을 최소화 |
...
높은 출력의 원전과 큰 계약 부하인 데이터센터를 직접 배치·건설하고, 두 시설이 같은 전력망과
하나의 냉각수 권역을 공유하게 만드는 것이다. 수랭은 전력을 아끼지만 물과 수원지 거리에
묶이고, 공랭은 물을 아끼지만 특히 폭염에 계통 피크를 키운다.

다만 1.0은 이 확장을 동시에 검증하지 않는다. 소형 가스발전, 배터리와 위기용 임대 피커만
사용해 전력망 건설·신뢰도 선택부터 완결한다. 원전·데이터센터·냉각수는 그 핵심이 사람
플레이에서 검증된 뒤 별도 milestone으로 열며, 석탄·풍력·태양열 같은 일반 발전 포트폴리오와
프로젝트 금융은 함께 끌어오지 않는다.
...
- 차단 우선순위는 P0→P1→P2→P3의 고정 규칙이다. 플레이어가 수치나 순서를 편집하지 않는다.
...
| 시나리오 기상계수 | 해당 시각의 선로·변압기 유효 정격; 풍향 등 하위 계수 없음 |
...
시나리오가 제공하는 하나의 기상계수로 유효 정격을 구하고, 양자화한 `흐름/유효 정격`의
제곱을 한 개의 고정 열 시정수로 누적한다. 일사율·복사율·도체별 대류계수·바람 방향은 1.0
파라미터가 아니다. 노후 회선에는 시나리오의 단일 고정 노후계수를 한 번만 적용한다.

```text
유효 정격 = 명판 정격 × 시나리오 기상계수 × 노후계수(대상 회선만 0.90, 그 외 1.00)
u = abs(양자화 흐름) / 유효 정격
H_next = H + (u² - H) / 60             # 1분 틱, 전역 시정수 60분
보호 차단 = u > 1.50 또는 H_next >= 1.00
```

`H`는 1 basis point 단위로 ToEven 반올림하는 0 이상 고정소수점 상태이며 부하가 낮아지면
같은 식으로 냉각된다. 회선을 새로
통전할 때 H는 0에서 시작한다. 60분과 차단 경계는 구조 고정값이고 자산별 override나 sweep을
허용하지 않는다.

변압기는 동적 열상태나 노후·정비계수를 갖지 않는다. 별도
`TransformerEffectiveRatingResolver`가 `명판정격×시나리오 기상계수`만 계산한다. 선로와
...
고정 폭염 계수와 공개 노후 계수는 허용흐름을 낮추고, 과부하 지수는 그 결과를 시간에 따라
...
1.0은 발전원 비교 게임을 함께 만들지 않는다. 플레이어가 건설할 수 있는 발전원은 소형
가스엔진 하나뿐이며, 배터리는 저장·예비력 선택을 담당한다. 피커와 병원 비상 디젤은 정해진
사건에서만 사용할 수 있어 별도 연료시장이나 발전 포트폴리오가 생기지 않는다. 아래 값은
첫 플레이테스트용 게임 수치이며 실제 시장가격이 아니다.

| 발전원 | 정격 | 기본 건설비 | 기본 공기 | 기동·램프 | 변동비 | 게임 역할 |
|---|---:|---:|---:|---|---:|---|
| 소형 가스엔진 | 80 MW | 48 M | 90일 | 45분, 40 MW/30분 | 0.115 M/GWh | 1.0의 유일한 건설 발전원 |
| 임대 가스 피커 | 30 MW | 6 M/30일 | 10분 동원 | 10분, 30 MW/10분 | 0.25 M/GWh | 3장 위기 전에만 계약 가능 |
| 배터리 | 35 MW / 70 MWh | 24 M | 40일 | 즉시 | 0.010 M/GWh 처리량 | 두 시간 예비와 시간 이동 |

캠페인은 기존 80 MW 가스엔진 한 기로 시작하고 1장에서 두 번째 가스엔진 부지 하나를
제공한다. 이 고정된 선택만으로도 `발전소 건설 → 송전 연결 → 공사 중 무매출 → 완공 뒤 매출`
루프를 검증할 수 있다. 피커는 상시 최적해가 되지 않도록 임대료와 높은 변동비를 모두 내며,
3장의 폭염 예보 전에만 계약할 수 있다. 배터리는 정상 가격차익보다 병원 절체와 2시간
지속공급을 우선한다.

1.0은 별도 기동비와 최소 운전·정지시간을 계산하지 않는다. 기동시간과 램프만 지키며 실제
...
정상 급전 순서는 `기존 가스 → 새 가스 → 배터리 충전/예약`으로 고정한다. 위기에는 살아 있는
가스 여유 → 배터리 → 계약한 피커 → 사전무장 공장 DR → P3/P2/P1 차단 순서를 쓴다.
...
배터리는 충전·방전 각각 94%의 고정 효율을 사용하고 1.0에서는 자기방전·열화·교체주기를
계산하지 않는다. 충전 전력은 최종 판매가 아니며 방전 뒤 수요처 경계에 전달된 양만 매출이다.
...
후속 우선 방향에서는 플레이어가 원전과 데이터센터를 모두 배치·건설한다. 원전은 큰 출력과
낮은 변동비를 주지만 냉각수와 송출망이 필요하고, 데이터센터는 큰 전력 수입과 높은 정전
위약을 준다. 두 시설은 하나의 `CoolingWaterZone`을 공유한다. 데이터센터가 수원지 hard cut
안에 있으면 운영 중 수랭↔공랭 전환이 가능하다. 수랭의 냉각수 요구는 수원지와의 직선거리에
선형 비례해 커지고, 공랭은 그 제약을 피하는 대신 특히 폭염에 최종 전력 수요를 늘린다.

1.0의 소형 가스발전은 허가가 끝난 고정 후보지 하나에만 지을 수 있고, 표시된 토지·허가비는
48 M 건설비에 포함된다. 원전·데이터센터·냉각수 데이터나 미래 interface는 만들지 않는다.
...
```text
최소화: 실제 건설비 + 발전 변동비 + 피커 임대 + DR + 수리 + 정전 보상
평가기간: 3장 캠페인의 실제 120일
제약조건:
  P0 미공급 에너지 = 0
...
미래 수명·잔존가치·할인율을 가정하지 않는다. 120일 동안 실제 지출하고 실제 공급한 값만
...
각각 제거한다. P0는 UPS가 고정 `10초 이하` 자동절체를 이어 받아 고객
...
`사용불가`, `수리중` 네 상태만 가진다. 캠페인 시작부터 `노후`와 폭염 취약 태그가 공개되며,
day 72에 경고, day 75 16:00에 사용불가가 되는 시간도 scenario 데이터에 고정한다. 플레이어가
사전에 예방정비를 끝내면 그 고정 사건이 취소되고, 아니면 같은 시각·구간에서 항상 발생한다.
저장·불러오기로 결과를 바꿀 난수는 없다.
...
첫 Balance Lab의 `폭염 수요증가분 공통 배수` 0.8/1.0/1.2는 위 세 수요 증가분에만 곱한다.
설비 감용은 고정하고 같은 sweep에 섞지 않는다.
...
3. day 72 경고 뒤 예방정비를 시작하지 않았으면 day 75 16시에 회선이 사용불가가 된다.
...
91%는 임의의 추가 감용이 아니다. 이 대표 운전점의 고정 폭염계수와 공개 노후계수를 한 번씩
...
| 수직 슬라이스 | 노후 선로 열화 | 공개 노후 태그, day 72 경고 | 고정 시각 사용불가 | 예방정비, 임시선 |
| 출시 핵심 | 공장 증산 | 계약 요청 | 급격한 지역 피크 | 새 배전 변전소·피더, 감축 계약 |
...
시작 포트를 고른 뒤 지형 위에 꺾임점을 놓는다. 커서가 움직일 때마다 총길이와 철탑 수,
...
A 공기 = ceil(7 + 3.8×6 + 강 횡단 22.2) = D-52
B 공기 = ceil(7 + 5.1×6 + 독립 단말 20) = D-58
```
...
154 kV 회선 감용 범위가 좁아진다. 회선의 `노후·폭염 취약` 태그와 day 72 경고, 예방정비를
하지 않을 때 day 75 16:00에 사용불가가 된다는 규칙은 공개되어 있다. 플레이어는 예방정비,
...
이때 허가 완료된 두 번째 소형 가스발전 후보지가 열리고, 90일 공사를 지금 시작해 3장에
맞출지 송전·병원 이중화에 현금을 남길지 선택하게 한다.
...
day 72에 명시적 경고를 내고 예방정비를 하지 않으면 day 75 16:00에 사용불가가 된다.
...
- 변압기는 `TransformerEffectiveRatingResolver`가 매 계산 경계마다 명판정격에서
  시나리오 기상계수만 한 번 계산한다. 상태·정비계수가 없고 직전 틱의 유효 정격에 다시
  곱하지 않는다.
...
- `OpenBreaker`, `CloseBreaker`, `SetBatteryReserveSoc`, `SetPeakerAutoStart`
...
검사를 강제 통과시키는 명령은 두지 않는다. P0/P1/P2/P3 차단 순서는 고정이며 접속 명령은
범위·용량·예비 피더가 실제로 유효한지만 바꾼다.

모든 명령은 성공·실패 영수증, `CommandId`, 전후 정규화 상태 해시, `QuoteId/ConstructionId`, 생성된
분개 ID, 비용과 완료예정일을 남긴다. 전체 상태 해시는 그래프뿐 아니라 현금, 열린 정전,
계량 커서, 공사 대기열, 배터리 SOC, DR과 고정 사건 상태를 포함한다.
...
저장에는 열린 정전, 미지급 보상, 마지막 계량 커서, 적용 완료 분개 ID, 공사 대기열,
...
L2는 1.0 구현 게이트의 필수 의존성이 아니다. L1 골든 시나리오가 안정된 뒤 필요하면
...
첫 개발단위는 **Static 타당성 게이트** 하나뿐이다. 아직 Godot 화면이나 전체 1분 캠페인을
만들지 않고, 기계 권위가 될 최소 catalog·scenario schema와 같은 산식을 읽는
`Gridworks.Headless balance`를 만든다. 대상은 손으로 작성한 네 전략 원형, 세 active knob의
27개 조합과 정상·폭염·단일고장·복구의 여섯 스냅샷이다.

이 단위의 완료조건은 단위·비용·공기 산식 테스트 통과, 동일 입력의 byte-identical 보고서,
물리적으로 불가능한 설계의 최초 실패 이유, 서로 다른 안전 설계 두 개 이상과 지배전략
경고다. 재미나 출시 통과는 선언하지 않는다. 완료 뒤 구현 난이도, 실행시간, 설명되지 않는
결과와 첫 사람 검토를 기록하고 다음 단위의 범위·파라미터·완료조건을 다시 쓴다.

그 이후 방향은 `Headless 시간축 → 2D 공간 건설 → 계통·위기 통합 → Godot 3D와 콘텐츠` 정도만
예약한다. 순서와 묶음은 [1.0 범위](SCOPE_1_0.md)의 게이트를 출발점으로 삼되, 앞 단위의 증거
없이 세부 작업목록이나 금융·원전·데이터센터·냉각수 기반을 미리 확정하지 않는다.

### 지도

- 6×6 km 계곡
- 기존 가스 발전소 1곳과 소형 가스발전 고정 후보지 1곳
- 마을 1, 병원 1, 공장 1
- 강 1, 공통위험 회랑 1
...
- 소형 가스엔진, 배터리. 가스 피커는 3장의 위기 대응용 임대 설비로만 등장
- 66 kV와 154 kV 선로
...
비교한다. `UnsafeSolve`는 이전 틱 흐름을 재사용하지 않고 안전 실패로 닫혀야 한다.
무작위 그래프에서는
...
2. **권위 Headless 실행**: 정적 검사를 통과한 12개 이하 후보만 실제 1분 틱, 열·기동·SOC,
   공사·정산·고정 사건으로 끝까지 재생한다.
...
한 milestone에서 동시에 여는 밸런스 knob는 3개, 절대 최대 6개다. 첫 수직 슬라이스는
`독립 경로 추가비`, `폭염 수요증가분 공통배수`, `정전 보상 공통배수` 세 개만 연다. 배터리와 임시 우회선
비용은 기준값에 고정한다. 각 knob의 하·중·상 3점 full grid는 27개이므로 복잡한 탐색 알고리즘이
필요 없다. 한 A/B 빌드에서는 한 파라미터 계열만 바꾼다. “점수가 오를 때까지” 무제한
최적화하지 않으며 목표 범위와
실행 예산을 먼저 고정한다. 자동 도구는 최소 두 개의 서로 다른 안전 설계, 가장 싼 위험설계와
가장 싼 안전설계의 비용 간격, 각 스트레스 케이스의 ENS·현금·캠페인 총비용·복구시간, Pareto 전면과
국소 민감도를 보고한다. 모든 케이스를 한 설계가 압도하면 콘텐츠나 수치가 닫힌 것으로 본다.

선로 리액턴스·손실·열상수·정격, 발전 효율, 배터리·임시선 비용, 공사시간, 판매요금과 고정 사건시각은 첫
milestone에서 sweep하지 않는다. 이 값에 문제가 있으면 밸런스 조정이 아니라 모델·scenario
수정으로 근거와 회귀테스트를 남긴다. 새 knob가 필요하면 기존 active knob 하나를 먼저 닫아
동시 탐색 상한을 유지한다.

원전·데이터센터·냉각수 확장의 숫자는 현재 모두 `TBD`이며 1.0 Lab에 넣지 않는다. 후속
milestone을 여는 adaptive checkpoint에서만 필요한 값과 처음 열 세 knob를 고른다. 냉각수
거리손실은 선형식 하나, 수원지 거리는 hard cut 하나, 냉각 상태는 수랭/공랭 둘로 제한하며
상세 원칙은 [1.0 이후 방향](POST_1_0.md)을 따른다.

사람의 플레이와 가상 정책의 차이는 제거할 잡음이 아니라 별도 증거다. 첫 Balance Lab에는
...
장기 확장은 이 정체성을 전력 밖으로 흩뜨리지 않고 더 날카롭게 만든다. 원전과 데이터센터를
배치한 뒤 공유 냉각수가 줄어들면, 물을 아끼는 공랭이 전력망 여유를 먹고 전력을 아끼는 수랭이
물 여유를 먹는다. 중요한 것은 물을 정교하게 시뮬레이션하는 일이 아니라 두 자원이 같은
송전망 설계를 서로 다른 방향에서 압박하게 만드는 일이다.
