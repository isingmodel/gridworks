...
- 서비스 권역의 정의문, 회랑별 정답, `같음/다름`, 추천·승자·총점은 participant 화면에 주지 않는다.
...
family가 필요하거나, 통과시키려면 fixture·정답 의미·게임 규칙을 바꿔야 하면 `PROXY-FAIL`이다.
`FAIL`은 게임 아이디어 폐기가 아니라 이 카드 version으로 다음 단계에 갈 증거가 부족하다는 뜻이다.
