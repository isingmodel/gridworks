...
2. `두 번째 심장`: 두 접속 회선, 선 교차와 실제 접속, 범람 안전 차단시험
...
예고한다. 서비스 권역은 변전소 접속 자격이며 발전원까지의 실제 경로를 대신하지 않는다. 병원 gate는
두 접속과 범람 차단시험 중 공급 유지를 확인하지만 두 회선의 완전한 독립성을 판정하지 않는다. 4장
진입 전 6개월 경과·열 초기화를 카드로 알리고, 실제 공급·미공급·남은 자금·약속만 결과에 쓴다.

final campaign save v3, 단계 D 저장의 결정론적 백업, 최근 공사·현재 장·이전 장 복구를 포함해
