...
현재 활성 구현 단계는 [2D 표현·사운드·패키징](docs/scopes/RELEASE_2D.md) 하나다. 세 장 캠페인
콘텐츠까지 고정된 현재 게임을 code-native 2D 표현, 최소 사운드와 macOS 내부 테스트 package로
마감한다. 전력망 규칙·콘텐츠를 더 만들거나 외부 테스트·공개 배포를 시작하지 않는다.
...
사운드와 최종 2D 표현·패키징은 현재 활성 단계의 범위 안에서만 구현한다.
