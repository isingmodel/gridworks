...
현재 활성 구현 단계는 [캠페인 골격·저장·기본 설정](docs/scopes/CAMPAIGN_SAVE_SETTINGS.md)이다.
완료된 누적 제품 흐름에 세 장 경계, 한 슬롯 저장·재개·장 재시작과 최소 화면 설정만 추가한다.
...
현재 단계 밖의 장별 콘텐츠 다듬기, 사운드와 최종 아트·패키징은 별도 단계가 열릴 때까지
구현하지 않는다.
