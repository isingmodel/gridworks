...
새 `CommercialMain`이 이제 기본 장면이다.

단계 E는 final campaign v2에서 `첫 불빛`, `두 번째 심장`, `두 번째 전원`, `북안의 약속`만 실제
플레이 가능한 연속 임무로 열었다. 변전소 서비스 권역은 접속 자격만 정하고 발전원까지 이어진 경로가
별도로 필요하다. 병원은 두 접속 회선과 범람 안전 차단시험을 요구하지만 두 회선의 완전한 전기·공간
독립성을 판정한다고 과장하지 않는다. 4장 진입 전 6개월 경과와 열 상태 초기화를 카드로 알리고,
