...
현재 활성 구현 단계는 없다. [2D 표현·사운드·패키징](docs/scopes/RELEASE_2D.md)까지 7단계 2D
개발을 마쳐, 세 장 캠페인을 code-native 2D 표현과 최소 사운드로 처음부터 끝까지 플레이하고
macOS 내부 테스트 ZIP으로 실행·저장·재개할 수 있다. 다음 단계인 외부 사용자 테스트, 지원 OS
확정, Developer ID 서명·공증과 공개 배포는 아직 열지 않았다.
...
[2D 완성 로드맵](docs/ROADMAP_2D.md)은 완료한 전체 순서,
[체크리스트](docs/ROADMAP_2D_CHECKLIST.md)는 단계 상태와 종료 증거를 소유한다. 완료된 범위는
다음 기능이나 외부 테스트·공개 배포를 자동으로 승인하지 않는다.
...
증명하지 않는다. 완성된 내부 후보에서는 사용자 승인으로 동일 prompt의
