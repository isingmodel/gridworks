...
제품 title의 `새 게임`은 누적 8장을 시작하고 모든 장의 stable 진행과 story-idle active event·duty
저장·재개를 지원한다. undelivered Core transition, queued/active story·result handoff와 완료 저장,
완료 후 선택, 제품용 audio·settings, 출시 패키지와 공식 UX 평가는 남아 있다.
...
| R2 save/Continue | product-owned journal-restorable 진행을 한 파일·exact replay·paused/no-modal 복원; active event·duty 지원, undelivered Core transition·queued/active story·handoff·완료 미지원 |
...
closing을 사용한다. product-owned session은 모든 장의 stable 진행뿐 아니라 story가 없거나 이미 닫혀
story flow가 idle인 active event·duty 진행도 저장할 수 있다. 두 경우 모두 active modal·epilogue·frame
debt가 없고 simulation이 Running 또는 PlayerPaused여야 한다. Core strict replay는 8장 stable 상태를,
session harness는 active event·duty의 exact 현재·미래 상태를, 별도 fresh-process smoke는 product
Continue의 paused 복원을 검증한다. 직전 exact standalone `FIRST_LIGHT` v1 save도 migration 없이 원
route로 이어 할 수 있다. undelivered pending transition, queued/active story·result handoff 자체와
완료 save, result/chapter/replay 선택은 없으므로 전체 제품 여정의 완성을 뜻하지 않는다.
...
실패 title smoke, 두 named checkpoint를 실행한다.
...
현재 R2에는 제품용 audio/settings, undelivered Core transition·queued/active story·result handoff·
완료까지 포괄하는 전체 campaign save/resume, 서명·공증된 패키지, 지원 OS 검증,
