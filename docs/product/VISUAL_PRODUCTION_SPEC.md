...
않는다. 현재 사실은 [Scope 0A 카드 테스트](../scopes/SCOPE_0A_CARD_TEST.md)가 권위다. Scope 0B가 승인된
뒤에도 새 활성 scope가 정한 machine-readable fixture가 인계검사를 통과한 시점에만 기계 숫자
권위로 승격한다.
...
- 활성 Scope 0A가 정의한 병원 P0와 현재 주 회로 E1
...
경로 초안에서는 가장 먼저 실패한 span을 거리와 함께 표시하고, 중간 지지물 추가 위치만
제안한다. 자동 최적 경로를 확정하지 않는다. 선이 교차해도 terminal 표식이 없으면 접속처럼
보이지 않게 높이·pattern을 분리한다.

철거 mode는 대상 설비, 함께 제거해야 하는 span, 철거비, 공사시간과 예상 미공급 고객을 한
묶음으로 강조한다. 통전 중이거나 필수 연결을 먼저 해제하지 않은 철거는 버튼을 비활성화하고
...
- `MaxSpan`을 넘은 연결과 필요한 중간 지지물이 즉시 읽히는가?
- 철거 대상과 함께 제거되는 span, 추가비·시간·미공급 결과가 확정 전에 보이는가?
