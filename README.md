...
현재 활성 구현 단계는 [출시판 재구축의 4단계 — 출시 마감](docs/scopes/RELEASE_REBUILD.md)이다.
새 `ReleaseMain`의 프롤로그 세 임무와 본편 다섯 장, 예산·사건·저장·재개와 장 재시작은 독립
검토까지 끝났다. 이제 같은 게임의 한국어, 2D 표현·사운드, 접근성, 종료 UX와 macOS 패키지를
출시 후보 수준으로 마감한다.
