...
범위는 [활성 Scope 0A](../scopes/SCOPE_0A_CARD_TEST.md)가 우선한다. 이 문서는 수치 fixture나
새 기능을 승인하지 않는다.
...
| 상태 | 의미 |
|---|---|
| `현재` | 활성 Scope 0A에서 실제로 사용하는 개념. 지금은 코드가 아니라 카드 fixture임 |
...
| 요소 | 현재 기획 판정 |
|---|---|
| 발전소 직접 건설 | `제품 방향·1.0 필수 능력`. 현재 Scope 0A/0B에는 기존 가스발전만 사용 |
| 변전소 직접 건설 | `제품 방향·1.0 필수 능력`. 현재 Scope 0A는 설치된 고정 변전소와 피더만 사용 |
...
현재 gate에서 위 기능을 사용할 수 있다는 뜻은 아니다. Scope 0은 authored 경로로 전력망 인과를
