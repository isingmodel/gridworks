...
| 전신주·철탑을 하나씩 직접 건설 | `현재 Scope 1`. 고정 두 terminal 사이 한 종류 support의 수동 배치만 구현·검증 중 |
| 각 오브젝트 사이 전선 길이 hard limit | `현재 Scope 1`. 인접 endpoint 사이 하나의 `MaxSpan` 규칙만 구현·검증 중 |
