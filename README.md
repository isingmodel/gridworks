...
새 `ReleaseMain`의 프롤로그 세 임무와 본편 다섯 장, 한국어 화면, 2D 표현·사운드,
접근성·종료 UX와 macOS 내부 후보의 기능 범위 구현은 끝났다. 이는 상용 표현·콘텐츠 품질 승인을
뜻하지 않는다. 자동검사와 저장소 밖 두 프로세스 전체 캠페인도 통과했다. 사용자 요청으로 수행한 직전 후보의
...
`16.4만원` 상태로 `BLOCKED` 종료했다. 후속 후보는 이 관찰에서 드러난 실제 경로 병목 오진,
화면 아래로 숨은 작업 버튼, 남아 있던 선로 도구, 선택 뒤에야 보이던 접속 한도를 고쳤다. 강물 위에는
새 변전소·전신주를 놓을 수 없고 선로만 가로지를 수 있도록 지형 규칙도 추가했다. 새 후보에 대한
한국어 원어민 소유자의 전체 플레이는 계속 기다리고 있다.
...
[상용 2D 게임 재기획서](docs/product/COMMERCIAL_2D_GAME_DESIGN_PLAN_KO.md)를 별도 제안으로 작성했다.
이 제안은 보이는 격자 없는 자유 배치와 전선·변압기·전신주 접속부의 국면별 열 한계를 상용
게임의 새 기반으로 삼는다. 현재 33×21 내부 후보를 소급 변경하거나 구현 권한을 자동으로 넓히지는
않는다.
...
새 출시판 기능을 구현했다는 뜻이 아니며, 현재 단계 밖의 구현을 자동으로 승인하지 않는다.
...
같은 33×21 지도와 전력망이 이어지는 프롤로그 세 임무와 본편 다섯 장을 목표로 한다.

1. 프롤로그 `첫 불빛 · 두 번째 심장 · 열돔 아래`: 건설, 이중화, 남부 전원 접속과 공유 용량을 짧게 익힌다.
2. `북안 확장`: 한 주간선에서 주거지와 정수장으로 분기한다.
3. `공유 구간`: 산업 수요가 공유 선로·분기점·변전소 용량을 경쟁한다.
4. `범람 예보`: 저지대 회랑 사고를 피해 고지대 우회망을 만들고 기존 망에 합류한다.
5. `계획 정지`: 다른 전원을 연결하고 합류망의 용량으로 계획정지를 견딘다.
6. `청류 비상전력`: 최대수요와 설비 사용불가를 종합해 도시를 지킨다.

최소 출시판에는 22.9 kV급 계획 모델, 3개 이상 연결되는 전신주·변전소, 선로·분기점·변전소의
현재 사용량과 정격, 결정론적 공급·우회, 촘촘한 지도, 전문적인 한국어 UX, 시작·사건·결과가
이어지는 이야기, 저장·설정과 설치 가능한 2D 빌드가 포함된다. 상세 범위와 현실성의 상한은
[활성 계약](docs/scopes/RELEASE_REBUILD.md)이 소유한다.
