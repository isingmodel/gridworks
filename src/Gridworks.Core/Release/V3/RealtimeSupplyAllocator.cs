...
public static class RealtimeSupplyAllocator
{
        ...
        ValidatedRequest interval)
    {
        ...
        foreach (ThermalLoadRequest loadRequest in interval.Request.Loads)
        {
            ...
            foreach (CommercialSourceDefinition source in context.OrderedSources)
            {
                ...
                {
                    PathCandidate? firstStaticPath = FindStaticShortestPath(
                        context,
                        source.NodeId,
                        load.NodeId);
    ...
    {
        quality = default;
        if (!path.NodeIds.Any(nodeId => IsEligibleSubstation(context, nodeId, load.NodeId)))
        {
            return Rejected(
                path,
        ...
        string? zeroCostNodeId)
    {
        ...
        {
            bool isSubstation = string.Equals(
                nodeId,
                substationId,
                StringComparison.Ordinal);
                     ...
                     .OrderBy(item => item.EdgeId, StringComparer.Ordinal))
        {
            ...
            ServiceRouteCost edgeCost = policy.EdgeCost(edge.EdgeId, lengthUnit);
    ...
    private sealed class ServiceFlowNetwork
    {
        ...
        {
            var forward = new ServiceFlowArc(
                to,
                _adjacency[to].Count,
                capacity,
                cost);
        ...
        public ServiceRouteCost? MinimumCostFlow(
            int source,
            int sink,
            int requiredFlow)
        {
            ...
            {
                ServiceRouteCost?[] distance =
                    new ServiceRouteCost?[_adjacency.Length];
                ...
                for (int pass = 0; pass < _adjacency.Length - 1; pass++)
                {
                    ...
                    for (int node = 0; node < _adjacency.Length; node++)
                    {
                        ...
                        for (int arcIndex = 0;
                             arcIndex < _adjacency[node].Count;
                             arcIndex++)
                        {
                            ...
                            ServiceRouteCost candidate =
                                distance[node]!.Value.Add(arc.Cost);
