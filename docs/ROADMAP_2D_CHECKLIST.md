...
| UX-87. 상용 게임 경험 | **활성 — `BLOCKED_PRE_CAPTURE`** | [활성 계약](scopes/COMMERCIAL_UX_87.md)·[고정 평가 프로토콜](product/COMMERCIAL_UX_EVALUATION_PROTOCOL_KO.md)·[native v1.1 부록](product/COMMERCIAL_UX_NATIVE_EVALUATOR_ADDENDUM_KO.md)·동결 v2 world/campaign | 26 story selector·17 typed failure, CommercialChecks 24/2,910 PASS; native schema 35·contract 53·gold 22·session 16·aggregate 76 PASS; candidate overlay 56-pair exact Core replay PASS; text v2 94.6625는 `SCORED_FORMATIVE`이지만 155 supported/22 partial로 `BLOCKED_EVIDENCE_VERIFICATION` | Gate C smoke는 개발 증거로만 PASS; checked-in gold owner 52 pending·E09 owner 4 unbound, 17 deterministic producer binding 미완료, 공식 cold/native 미실행 | `NOT_COLLECTED`; LLM은 사람 증거가 아님 | Gate C exact fix 독립 재검토 P0/P1/P2 0; pre-capture integrity follow-up 재검토 예정, producer를 닫은 뒤 3 cold actor+coverage·3 blind judge, official `CommercialUXProxy >=87` |
