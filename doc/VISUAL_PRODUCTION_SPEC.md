...
않는다. 현재 사실은 [Scope 0A 카드 테스트](SCOPE_0A_CARD_TEST.md)가 권위다. Scope 0B가 승인된
뒤에도 fixture 인계검사를 통과한 시점에만 `data/scenario.json`을 기계 숫자 권위로 승격한다.
...
- 화면 구석의 선형 타임라인: 현재, `DAY 3 08:00` 두 번째 회로 기한,
  `DAY 9 16:00~20:00` 강변 통로 사용불가
- UPS 15분 + 디젤 4시간 45분
...
```text
두 안 모두 기한 안에 완공 · E1과 다른 차단 회로

강변 병행 회랑         북부 우회 회랑
8 M                    12 M
기존 주 회로 옆         북쪽 언덕의 별도 통로
같은 강변 통로
```
...
- 왼쪽 위의 선형 예고 타임라인은 현재부터 다음 의무·수요증가·폭염·사용불가·복구를 시간순으로
  표시한다. 점은 시각, 막대는 기간이며 새 경보와 마감에서 자동 정지한다.
