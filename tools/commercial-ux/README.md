...
- 누적 8장 product의 모든 장 stable 진행, story-idle active event·duty save/Continue와 exact prior
  standalone `FIRST_LIGHT` v1 Continue: deterministic wiring 완료
...
save codec의 누적 8장 stable strict replay와 pending fail-closed를 확인할 때:
...
검사한다. `./dev check`는 Core의 누적 8장 stable replay와 pending fail-closed, isolated two-process의
active event·duty product save-create→Continue, exact prior standalone `FIRST_LIGHT` Continue와
invalid/unsupported/I/O-failure title 상태를 함께 검사한다. queued/active story·result/handoff·완료를
...
undelivered Core transition·queued/active story·result/handoff·완료를 포함한 전체 save/resume와 완료 후 선택,
