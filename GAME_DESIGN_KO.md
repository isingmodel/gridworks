...
콘셉트 스케치 재현 방법은 [비주얼 제작 명세](VISUAL_PRODUCTION_SPEC.md)를 따른다.
...
| 한 세션 | 캠페인 임무 45~90분, 샌드박스는 제한 없음 |
...
- 겹치는 필드의 건물은 기본적으로 여유가 큰 변전소에 붙고, 플레이어가 수동 고정할 수
  있다. 자동 재배치는 히스테리시스를 사용해 매 틱 흔들리지 않게 한다.
...
600 MW 정격을 지킨다. 이 송출 패키지가 시운전되지 않으면 원전도 상업 운전할 수 없다.
...
기록되지만, 비용 등급은 자산 CAPEX를 한 번만 넣고 금융비에는 이자·발행 수수료만 넣는다.
원금을 CAPEX와 금융비에 두 번 더하지 않는다.
...
비용 등급은 자금 출처와 무관하게 통합 패키지 전체 CAPEX를 사용하므로 보조금으로 사회적
비용을 숨길 수 없다.
...
CAPEX를 이미 평가하므로 비용 등급에 원금이나 CAPEX를 다시 더하거나 빼지 않는다. 투자자
배분도 같은 자산의 소유권 이전이므로 효율 LCC에서는 제외하고 지급능력·기말현금에는 전액
반영한다.
...
| 데이터센터 | 25~90 MW | 거의 일정, 냉각 영향 | 높음 | 전력 품질과 이중 공급 계약 |
...
않는다. 주민기금·법무·허가비는 잔존가치와 O&M 기준 자산에 포함하지 않는다. 더 먼 부지를
...
영업 현금흐름 = 전력 판매 매출 - 발전 변동비(연료·소모품 포함) - 고정 O&M
...
보상은 정전이 끝나야 생기는 비용이 아니다. 매 틱 미지급 보상채무를 발생시켜 다음 일일
...
계량기는 1분 틱마다 공급·미공급 MWh와 발전원별 변동비를 누적한다. 사용 요금, 용량 계약,
...
직접 취득 토지는 잔존가치 대상이지만 O&M 기준에서는 제외한다.
...
```text
최소화: CAPEX + OPEX + 발전 변동비·외부조달 + 금융 이자·수수료 + 정비
        + 정전 보상·계약 비용 - 종료시 잔존가치
...
물리자산 잔존가치는 위 표의 수명을 사용해 `실제 지급된 적격 물리 CAPEX × 남은 설계수명
비율 × 상태/100 × 0.6`으로 계산한다. 적격 물리 CAPEX는 발전·송전·변전 본체와 영구 안전·
환경설비다. 직접 취득 토지는 수명·상태를 쓰지 않고 `실제 직접 토지 취득비 × 지도별 회수
계수`로 따로 계산하며 기본 회수계수는 0.6이다.

주민기금, 주민·영업 영향 보상, 허가·소송비, 금융비, 벌금, 정전 보상, 임시선과 철거·임대비는
O&M과 잔존가치 기준에서 제외한다.

아직 시운전되지 않은 자산은 일반 잔존가치를 받지 않고 `회수 가능한 현장 자재비 - 철거·
계약해지비`만 받는다. 단독 임무 종료 때 남은 지급약정은 종료비용에 포함하고, 캠페인에서는
공사·약정·부채를 다음 장으로 그대로 이월한다. 이렇게 막판 미완공 공사로 비용을 숨길 수
없다.

기본 N-1 검사는 핵심 선로 하나, 변압기·변전소 하나, 가장 큰 온라인 발전기 하나를 각각
제거한다. 각 경우 P0는 같은 1분 시뮬레이션 틱 안에 자동 절체되어 미공급이 없어야 하고,
P1은 30분 안에 95% 이상으로 회복해야 한다. 남은 설비는 120% 초과를 15분 넘게 지속하면
안 되며 60분 안에 모두 100% 이하로 복귀해야 한다. 캠페인 1~2장의 튜토리얼만 이 안전
게이트를 명시적으로 면제한다.
...
단일 점수 대신 `전력 판매 매출`, `순영업 현금`, `총비용`, `정전 보상금`, `미공급 에너지`,
`병원 정전 시간`, `최대 부하율`, `복구 시간`을 나란히 기록한다. 매출은 생존과 확장에
필수지만, 같은 의무를 덜 안전하게 떠안아 점수를 높이지 못하도록 효율 등급은 고정된 의무
수요의 생애주기 비용으로 비교한다.

캠페인의 행정 종료 조건은 안전 등급과 별도다. P0 연속 정전 15분 또는 회복 불가능한
광역 정전 6시간이 넘으면 캠페인이 종료될 수 있다. 그보다 짧은 P0 정전도 즉시 안전
게이트에는 실패하지만, 플레이어는 복구를 계속하고 사고 보고서를 받을 수 있다.
...
- 과부하: 반복 경고음보다 주파수 흔들림과 팬·변압기 소리가 먼저 변함
...
- 청백색 선은 실제로 통전된 송전 경로다.
- 반투명 청색 권역은 선택한 변전소가 담당할 수 있는 서비스 필드다.
- 호박색 유령 철탑은 아직 돈과 시간이 필요한 계획 노선이다.

어두운 외곽과 점등된 마을의 대비가 설비 의존성을 보여준다. 좌측 건설 팔레트, 상단 예산·
용량, 우측 설비 상태, 하단 시간 제어가 기본 HUD를 이룬다.

### 15.2 송전 경로 비교 화면

![송전 경로 A/B — 비용·공기·철탑 수·피크·N-1 비교](assets/03-route-comparison.png)

A는 3.8 km·12철탑·18.4 M·52일이고, 강 횡단 접근부에서 기존 회랑과 공통 고장 구간을
공유하므로 `N-1: NO`다. B는 5.1 km·15철탑·19.6 M·58일이며, 별도 발전 측 모선과 도로
회랑으로 접속해 `N-1: YES`다. 즉 N-1 차이는 단순히 길이가 아니라 공통 모선·회랑의
유무에서 나온다. 지도 위 유령 철탑, 꺾임점과 경고·방패 표식이 우측 견적 카드에 직접
대응한다.
...
이 화면은 작업반 한 개씩을 쓴 보수적 견적이다. 구간 병렬 작업반을 임대하면 DAG를 다시
계산해 완공일만 줄고 자재비는 유지된다.

B의 도로는 통행을 계속 유지하는 공공도로라 접근 비용 `×0.9`만 받고 전용 진입권의 공기
`×0.85`는 받지 않는다. 따라서 위 `D-58`이 §6.2의 예외 규칙과 일치한다.

### 15.3 폭염 + 노후 선로 사용불가 화면

![폭염과 노후 송전선 사용불가 — 임시 우회 공급](assets/02-heatwave-outage.png)

기존 상부 회랑은 전류가 끊기고 정비 표식과 국소 적색 진단선이 나타난다. 미리 지은 하부
우회선은 청백색으로 통전되지만 일부가 호박색으로 변해 용량 한계에 가까움을 알린다. 병원은
서비스를 유지하고, 외곽 마을과 공장은 감축 후보가 된다. 상단 `54/58 MW`는 현재 수요와
살아 있는 공급력이다. 이 공급력은 표시된 임시선 `34/35 MW`와 수요지 측 가스·배터리
24 MW의 합이다. 우측 `0/180 MW`는 사용불가가 된 154 kV 회선의 흐름과 명판 정격이다.
폭염은 설원 미학을 복제하지 않고 백색 태양, 구리색 먼지와 마른 지면으로 같은 생존 압박을
만든다.

### 15.4 발전소 입지·님비 비교 화면

![원전 후보지 A/B — 님비 할증과 계통 접속비 비교](assets/04-plant-siting.png)

마을에서 2.4 km 떨어진 A는 송전선이 짧지만 원전 건설비가 `×2.5`가 되어 본체 420 M에
입지 할증 630 M이 붙는다. 2×345 kV 송출망과 1,200 MW 허브 90 M까지 합한 총액은
1,140 M, 완공은 630일 뒤다. 6.8 km 떨어진 B는 님비 할증이 없지만 긴 복수회선·특수경간·
허브에 180 M이 들어 총 600 M이며, 664일 뒤 완공된다. 거리 띠는 서비스 필드가 아니라
토지·허가·주민 노출을 설명하는 입지 분석 오버레이다. 색 띠는 위험 구간 범례이고 실제
가격은 §7.3의 연속 보간값을 사용하므로 경계에서 비용이 갑자기 뛰지 않는다.

두 후보는 캠페인이 제공한 공공 입찰 부지라 직접 토지비가 0이고, 이 비교에는 추가 완화책도
없다. 따라서 우측 `PLANT + SITING + GRID = TOTAL` 세 줄만으로 총액이 정확히 닫힌다.

이 비교는 먼 부지가 언제나 정답이라는 뜻도 아니다. B의 긴 회랑은 손실, 철탑별 고장 노출,
정비비와 N-1 보강비를 늘린다. 우측 견적은 건설 승인 전 확정값을 보여주며, 이후 운영비와
위험은 생애주기 비용 미리보기에서 별도로 비교한다.

통합 패키지 자기자본 20%는 A 228 M, B 120 M이다. 상단 현금 120 M에서 B만 즉시 확정할
수 있어 청색 `BUILD B`가 활성화되고, A는 `INSUFFICIENT`로 잠긴다. A를 택하려면 일반 신용과
공공 공동투자 조건을 먼저 확정해야 한다. 발행 채권과 자기자본은 본체만이 아니라 필수 송출망
완공에도 묶인다.

> 1~3번 스케치는 수직 슬라이스, 4번은 캠페인 장기 투자 기준의 UX 예시다. 최종 밸런스는
> 플레이테스트 뒤 조정하되 같은 화면 안에서 분자·분모의 의미는 바꾸지 않는다.

## 16. 캠페인 구조

출시 캠페인은 한 지역이 작은 마을망에서 복원력 있는 광역망으로 성장하는 7장 구조다.
...
| 6 | 값싼 길의 대가 | 메시망, N-1, 배터리 | 발전기 정비 + 폭풍 |
...
원전 장기입찰은 day 0에 열리고 day 45에 최종 산업계약이 확정된다. day 700 열돔 전에
상업 운전하려면 A는 `700-630 = day 70`, B는 `700-664 = day 36`까지 확정해야 한다.
따라서 B는 600 M으로 싸지만 수요가 확정되기 전에 되돌리기 어려운 투자를 해야 하고, A는
1,140 M으로 비싸도 day 45의 계약 결과를 본 뒤 결정할 수 있다. 마지막 착공일은 후보 카드에
항상 표시한다.
...
게임의 권위 시뮬레이션은 다음 기반 모듈로 구성한다.

- 결정론적 명령, 영수증, 스냅샷, 저장과 리플레이 계약
- 손실을 포함한 전력조류와 섬 탐지
- 선로 열 누적, 보호 차단, 주파수와 부하 차단
- 발전기, 저장장치, 중요 부하와 수요반응 모델
- 경제 원장의 분개·잔액 저장과 공사 생명주기
- 꺾임점 기반 노선 길이·비용 계산
- Headless 검증과 Unity 비의존 시뮬레이션 경계
...
- 현금흐름과 분리해 CAPEX·경제비용·잔존가치를 계산하는 `EvaluationLedger`
...
- 매 틱 명판정격에서 `기상×상태×정비`를 새로 곱하는 `EffectiveRatingResolver`. 전 틱의
  유효 정격에 다시 곱하지 않음
- UI Toolkit 기반 건설·진단 화면
...
- `OpenBreaker`, `CloseBreaker`, `SetDispatch`, `SetReserveFloor`
...
모든 명령은 성공·실패 영수증, `CommandId`, 전후 전체 상태 해시, `QuoteId/ProjectId`, 생성된
...
수직 슬라이스는 한 번에 전부 만들지 않고 아래 세 게이트로 자른다.

1. **공간·견적 게이트**: 설비 배치, 서비스 필드, 자유 노선, 전신주 자동 배치, 비용·공기
   비교와 공사 완료까지만 구현한다.
2. **계통·운영 게이트**: 동적 그래프, 전력조류, 용량·손실·열, 발전·저장, 부하 우선순위와
   N-1 미리보기를 붙인다.
3. **위기·지속성 게이트**: 노후도 불확실성, 점검·정비, 폭염, 사용불가·복구, 경제 평가,
   저장과 리플레이를 붙인다.
