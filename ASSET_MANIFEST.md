...
|---|---|---|
| 청류시 지형 tile 7종 | `game/art/commercial/tiles/`의 지면 4종·강·주거 block·의료원 block PNG. 네 concept은 style reference로만 사용했고 각 파일의 prompt·SHA-256은 [`commercial-map-assets-v1.prompts.md`](game/art/commercial/commercial-map-assets-v1.prompts.md)에 고정 | OpenAI built-in ImageGen의 Gridworks 프로젝트용 생성 출력. 공개 라이선스 없음. 내부 후보에만 포함 |
| 망 설비·시설 object 9종 | `game/art/commercial/objects/`의 발전 접속 설비·일반/보강 전신주·교량 기초·변전소·주거/의료/정수/산업 transparent PNG. 파일별 prompt·SHA-256과 alpha 검수는 같은 [생성 기록](game/art/commercial/commercial-map-assets-v1.prompts.md)에 고정 | OpenAI built-in ImageGen의 Gridworks 프로젝트용 생성 출력. 공개 라이선스 없음. 내부 후보에만 포함 |
