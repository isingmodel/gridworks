...
활성 또는 완료 gate의 participant-facing 카드와 진행 자료 같은 파생 실행 산출물은 루트 [`playtests/`](../playtests/)에 둔다. 이 자료는 해당 scope를 실행·기록할 뿐 규칙·숫자 권위가 아니다. 원답은 `playtests/**/private/`에만 두고 Git에서 제외한다.
...
| 완료된 authored playable의 정확한 계약은 무엇인가? | [Scope 0B 계약](scopes/SCOPE_0B_PLAYABLE.md) | 제품 문서나 후보 TODO가 역사 범위를 늘리지 않음 |
