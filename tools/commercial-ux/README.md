...
- 누적 8장 product의 모든 장 stable in-progress save/Continue와 exact prior standalone `FIRST_LIGHT` v1
  Continue: deterministic wiring 완료
...
save codec의 누적 8장 stable strict source/journal replay를 확인할 때:
...
검사한다. `./dev check`는 Core의 누적 8장 stable replay, isolated two-process의 product
save-create→Continue, exact prior standalone `FIRST_LIGHT` Continue와 invalid/unsupported/I/O-failure title
상태를 함께 검사한다. 사건·duty·story·result/handoff·완료를 포함한 전체 save/resume와 전체 8장
production-input 여정은 별도 fresh-process E2E가 필요하다.
...
transient·result/handoff·완료를 포함한 전체 save/resume와 완료 후 선택, audio/settings와 fresh-install
candidate gate를 먼저 닫는다. 이 package gate가 current R2 candidate packager, finalized manifest와
verifier를 소유한다.
