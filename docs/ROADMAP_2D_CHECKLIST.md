...
`필요`는 해당 단계 종료 전에 외부 formative observation을 기록한다는 뜻이다. `선택`은 자동검사와
native 확인 뒤에도 사람만 답할 질문이 남을 때만 수행한다는 뜻이다. 참가자 수나 통과율 목표는
두지 않는다.
...
- 이 단계에 필요한 외부 관찰을 완료했거나, 선택 사항이면 생략 이유를 기록했다.
