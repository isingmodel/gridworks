...
현재 활성 구현 단계는 [출시판 재구축의 2단계 — 출시판 지도와 건설 UX](docs/scopes/RELEASE_REBUILD.md)다.
1단계의 새 world loader, 분기·합류 공급 평가기와 대표 병목·우회 검사는 독립 검토까지 마쳤다.
이제 그 규칙을 계산하지 않는 새 `ReleaseMain`에서 33×21 지도, 설비·선로 공사와 전문적인 한국어
정보 위계를 구현한다. 캠페인 데이터·저장과 본편 장 진행은 아직 3단계 범위다.
...
내부 후보에 대한 소유자 전체 플레이 피드백은 2026-08-17 수집했다
(`OwnerPlayReviewStatus = COLLECTED`). 이 검토에서 출시 차단 수준의 표현·망 모델·콘텐츠 문제가
확인됐으며, 새 출시판의 외부 formative test는 아직 수집하지 않았다
...
1. 프롤로그 `첫 불빛 · 두 번째 심장 · 열돔 아래`: 건설, 이중화, 발전·정비를 짧게 익힌다.
