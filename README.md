...
현재 활성 구현 단계는 [세 장 캠페인 콘텐츠 고정](docs/scopes/CAMPAIGN_CONTENT.md)이다. 완료된
전력 규칙과 저장 골격은 그대로 두고 각 장의 브리핑·목표, carry-over 최소현금과 softlock 방지만
고정한다.
...
현재 단계 밖의 사운드와 최종 아트·패키징은 별도 단계가 열릴 때까지 구현하지 않는다.
