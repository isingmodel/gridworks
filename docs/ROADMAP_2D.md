...
현재 B~G.3은 완료됐다. G.3의 최종 v27은 원래 자동 점수 gate를 넘긴 것으로 간주하지 않고,
사용자의 명시적 종료 지시에 따라 성공 기록으로 닫았다. 사용자는 2026-08-22에 로드맵 H와 별개인
[상용 UX 87 개선 범위](scopes/COMMERCIAL_UX_87.md)를 열었으며, 현재 활성 구현 권한은 그 계약만
소유한다. H는 자동으로 이어지는 다음 작업이 아니라 외부 입력과 별도 승인이 필요한 게이트다.
현재 제품은 1920×1080 UI 100%·125%만 지원하며 1280×720/720p는 지원하지 않는다.
