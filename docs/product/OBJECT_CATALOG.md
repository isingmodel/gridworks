...
범위는 루트 [README](../../README.md)가 지목한 활성 scope 또는 terminal state가 우선한다. 이 문서는 수치 fixture나
새 기능을 승인하지 않는다.
...
| 상태 | 의미 |
|---|---|
| `현재` | 루트 README가 지목한 활성 scope 또는 가장 최근 완료 prototype에서 실제로 사용하는 개념 |
| `제품 방향` | 완성 제품이 지향하지만 아직 구현이 승인되지 않은 기능 |
| `조건부 후보` | 사람 증거와 별도 gate 승인 뒤에만 열 수 있는 기능 |
...
| 요소 | 현재 기획 판정 |
|---|---|
| 발전소 직접 건설 | `제품 방향·1.0 필수 능력`. Scope 0 후보 경계에는 기존 가스발전만 사용 |
| 변전소 직접 건설 | `제품 방향·1.0 필수 능력`. Scope 0 후보 경계에는 설치된 고정 변전소와 피더만 사용 |
...
| 공장 | authored 산업 수요처, 전력회사 소유 아님 | 수요와 향후 계약 감축 후보 확인 | 큰 부하, 증설·교대 수요사건, P3/DR 후보 | 건설·철거·무상 수요반응 강제 |
...
검증이라 직접 건설 UI가 없고, 완료된 Scope 0B도 고정 피더와 authored 회랑만 사용했다.
...
1.0의 조건부 후보이며 철거와 다른 행위다. 완료된 Scope 0에는 플레이어 수리 명령도 없었다.
