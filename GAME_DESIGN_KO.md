...
1.0의 권위 시간 모델은 하나의 `AdvanceOneMinute()`뿐이다. 달력 고속 진행도 시간을
...
같은 명령·외부사건 입력이면 화면을 그리는 실시간 실행과 무렌더링 고속 실행의 canonical
state hash가 같아야 한다. 프로파일링 뒤 실제 병목이 확인될 때만 변화 없는 하위 시스템의
...
6. 3~5를 정확히 4회 반복한다. 네 번째 반복 뒤 최대 모선 전력수지 잔차 또는 직전 반복 대비
   총손실 변화가 0.001 MW를 넘으면 `UnsafeSolve`, 아니면 흐름을 1 kW 단위로 양자화한다.
7. 부족과 잉여를 아래의 서로 다른 순서로 해소한다.
```

`ReferenceBusResolver`는 살아 있는 외부 연계선, grid-forming 가능한 온라인 발전기의 정책
우선순위, 가장 작은 안정 `AssetId` 순으로 기준 모선을 고른다. 공급원이 없는 섬은 무전압
섬으로 분류해 행렬을 풀지 않는다. 모선과 행렬 행은 안정 ID 순서로 만들고 관리 코드의 고정
선형해법과 고정 피벗 동률 규칙을 쓴다. 행렬 특이, NaN 또는 비수렴이 발생하면 이전 틱의
흐름을 재사용하지 않고 `UnsafeSolve`로 전환해 해당 섬의 신규 송전을 금지하고 진단을 남긴다.
...
도달하거나 안전 상태를 만들지 못하면 `UnsafeSolve`로 실패하고 조용히 부분 결과를 확정하지
않는다.
...
선로 열은 고정 폭염 배수를 매분 다시 곱하지 않는다. `LineThermalModel`은 전류제곱 발열,
일사, 주위온도와 풍속에 따른 대류·복사 냉각을 축약한 1차 열상태로 도체온도를 갱신하고,
정상·비상 정격과
차단까지 남은 시간을 만든다. §11.2의 감용 퍼센트는 첫 밸런스의 대표 운전점 앵커이며,
실제 런타임은 같은 날에도 바람과 흐름에 따라 유효 정격이 오르거나 내릴 수 있다.

선로의 날씨는 `LineThermalModel`에서만 적용한다. 노후 상태는 저항, 허용 최고온도 또는
냉각 성능 파라미터를 바꾸며, 그 결과에 다시 `기상×상태` 정격 배수를 곱하지 않는다.
변압기는 동적 선로열 모델을 쓰지 않고 `명판정격×기상계수×상태계수×정비계수`의 별도
`TransformerEffectiveRatingResolver`를 사용한다. 이 둘을 같은 resolver에 넣지 않는다.
...
기온, 바람과 노후도는 위 열상태를 통해 허용흐름과 냉각 속도를 바꾼다. 따라서 평상시
82%인 선로가 폭염에는 96%가 될 수 있다.
...
|---|---:|---:|---:|---|---:|---|
| 소형 가스엔진 | 80 MW | 48 M | 90일 | 15%, 45분 | 0.115 M/GWh | 1.0의 초기 주력, 적당한 마진과 유연성 |
...
화력 안에서도 역할이 다르다. 소형 가스엔진은 1.0 지역의 평상시 공급을 맡고, LNG 피커는
3장 위기 때 임대·해금되는 비싼 대응자산이다. 대형 LNG는 출력 조정과 기동이 빠르지만 연료비가 높고,
...
| 설비 | 램프 | 기동비 | 최소 운전 / 정지 |
|---|---:|---:|---:|
| 소형 가스엔진 | 40 MW/30분 | 0.30 M | 3시간 / 1시간 |
...
| 구간 | 대표 수단 | 의미 |
|---|---|---|
| 즉시~1분 | UPS, 배터리, 비상 디젤 | 사고 직후 P0와 계통 불균형 보호 |
...
요구 MW와 마진을 함께 보이고, 좁은 화면의 `RESERVE 6%`는 세 구간 중 가장 낮은
`ReserveMarginPercent`다. `LIMITING: 2H`처럼 제약 시간대를 함께 표시한다. 비자발적 자동
...
| 영구 변전소 | 35년 |
| 소형 가스엔진 / 가스 피커 / 복합화력 / 석탄 / 원자력 | 25 / 30 / 35 / 40 / 60년 |
| 태양광 / 풍력 | 25 / 25년 |
| 집광형 태양열 | 35년 |
| 배터리 | 15년 |
| 비상 디젤 | 20년 |
...
밸런스 값은 각각 6 M/10 M이다. 옵션대금은 통합 패키지 총액과 플레이어 자기자본 20% 안에
포함된 선납 자기자본이며 별도 가산비가 아니다. 최종 투자하면 첫 30% 마일스톤의 플레이어
자기자본에서 차감한다. 따라서 A는 총액 1,140 M·자기자본 228 M 중 6 M을 먼저 내고
222 M이 남으며, B는 총액 600 M·자기자본 120 M 중 10 M을 먼저 내고 110 M이 남는다.
채권 약정은 여전히 통합 패키지 총액의 80%이고 옵션대금을 빚으로 다시 인출하지 않는다.
포기하면 옵션대금은 매몰비용으로 남고 돌려받지 못한다. 옵션 만료 전 최종 투자결정을 하면
이미 지난 승인기간을 그대로 인정한다. 따라서 플레이어는 전체 자기자본을 조기에 묶는 대신
제한된 비용으로 “기다릴 권리”를 산다. 옵션이 없으면 최종 투자결정일이 곧 모든 선행승인의
시작일이다.
...
다르다. 견적 화면은 이 차이를 `초기 현금`, `평시 일수익`, `지정 스트레스 비용`, `완공일`로
나눠 보여주고 하나의 불투명한 총점으로 숨기지 않는다.

`지정 스트레스 비용`은 확률을 임의로 곱한 기대값이 아니다. 시나리오가 이름, 시작상태,
날씨·수요 이력과 제거할 설비를 고정한 하나 이상의 스트레스 케이스를 같은 견적에 적용해
추가 연료·수입·DR·보상·수리비를 합산한 값이다. UI는 예를 들어 `폭염 피크 + 회랑 A 사용불가
...
최종 투자·발주 승인 순간에는 **아직 지급하지 않은 총 약정액 100%를 가용 현금·신용·승인된
전용 채권에서 예약**한다. 개발 옵션만 산 단계에는 옵션대금만 지불·예약하며 최종 투자 뒤에는
통합 패키지 총액에서 선납 옵션대금을 뺀 잔액을 예약한다.
전용 채권과 공동투자 약정은 프로젝트별 제한계정이라 다른 공사의 담보가 되지 않는다.
마일스톤 금액 `q`의 분개는 아래처럼 한 거래로 처리한다. 공동투자가 없으면 `c=0`, 있으면
승인된 자기자본 분담률만큼이다. `o`는 통합 패키지에 이미 낸 옵션대금 중 아직 상계하지 않은
금액이다.

```text
b = 0.8q                              // 채권 몫
c = 승인된 공동투자 몫              // 최대 0.1q
o = min(남은 옵션 선납크레딧, q-b-c)
남은 프로젝트 지급약정 -= (q-o)      // 지금 실제로 남아 있던 청구액만 감소
미인출 전용채권 -= b; 인출 채권원금 += b; 현금 += b
미인출 공동투자약정 -= c; 공동투자 유입 += c; 현금 += c
남은 옵션 선납크레딧 -= o
현금 -= q-o
플레이어의 이번 마일스톤 순현금 감소 = q-o-b-c
...
1.0의 소형 가스엔진이 50 MW를 하루 공급하면 주거 매출 0.168 M, 변동비·탄소비용
`1.2×(0.115+0.012)=0.1524 M`, 80 MW 전체가 가용할 때 용량수입 0.024 M, 일 O&M 약
0.0033 M이다. 손실·기동비 전 마진은 약 0.0363 M으로 양수다. 같은 역할을 가스 피커가
맡으면 구조적 적자가 되므로 피커는 평상시 주력이 아니라 위기·피크 수입으로 비용을 회수한다.
...
평가 자본비용 = Σ_j [적격 물리자산 지급액_j × CRF(8%, n)
                         × (평가종료일-지급일_j) / 365]
토지 기회비용 = Σ_j [직접 토지 지급액_j × 연 3%
                         × (평가종료일-지급일_j) / 365]
```

30/40/30 마일스톤은 각각 실제 지급일부터만 노출되므로 첫 지급일에 전체 CAPEX가 집행된
것처럼 과대평가하지 않는다. 미인출 약정의 기회비용은 이미 약정수수료에 잡히며 평가 CAPEX에
다시 넣지 않는다. 상태 저하는 잔존가치 할인으로 숨기지 않고 정비비, 유효 정격, 고장위험과
종료 시 시나리오 기준상태까지 회복하는 결정론적 `미이행 정비견적`으로 별도 평가한다. 주민기금,
...
정전이 6시간을 넘으면 캠페인이 종료될 수 있다. 1.0은 모선 충전 순서나 위상 동기화를 직접
조작하게 하지 않지만, 외부 연계선·가압된 섬·`BlackStartCapable` 비상전원 중 하나가 살아
있으면 추상 블랙스타트를 제공한다. 해당 전원이 고정 기동시간을 마치면 하나의
`EnergizedIsland`가 생기고, 플레이어가 적격 차단경로를 연결하면 시스템이 전압등급·고장구간·
동기조건을 자동 검증해 섬을 합친다. 기동 중에는 예상 가압시각과 복구 가능한 MW를 표시한다.
그 어떤 기동원도 없이 전체 계통이 무전압이 되면 `회복 불가능`으로 판정해 임무를 종료한다.
그보다 짧은 P0 정전도 즉시 안전 게이트에는 실패하지만, 플레이어는 복구를 계속하고 사고
보고서를 받을 수 있다. 직접 모선 충전과 수동 섬 동기화는 장기 확장 범위다.
...
- `ProtectionZoneMembership[]`: 한 보호동작으로 함께 차단되는 범위들의 집합
- `RiskGroupMembership[]`: 산불·강풍·산사태·공사에 같이 노출되는 공간 위험집합
- `SiteCommonModeId`: 침수·화재처럼 같은 부지를 묶는 공통위험

긴 회선은 `LineSection[]`으로 나누고 각 구간이 여러 보호구역과 위험집합에 동시에 속할 수
있다. 자산 전체에 단일 회랑 ID를 붙여 교량·산사태·공사구간 중 하나만 공유하는 두 노선을
전 구간 공통위험으로 과장하지 않는다. `독립 N-1`은 전기적 단일 고장을 뜻하고, 같은 회랑의 두 회선이 함께 손상되는 사건은
...
외생 표본은 하나의 순차 난수열이 아니라
`ScenarioSeed + RandomStreamId + EventType + AssetId + TimeIndex + OccurrenceIndex`를 키로 한
counter-based 표본에서 만든다. 권위 스트림은 `Weather`, `Demand`, `AssetFailure`,
`InspectionNoise`, `ScenarioVariation`으로 분리한다. 자산을 하나 추가하거나 정책이 다른 수의
표본을 소비해도 기존 날씨·수요·자산의 잠재 고장표본이 바뀌지 않는다. 파티클·소리 변형 같은
`Cosmetic` 난수는 별도 비권위 스트림을 사용하고 저장·리플레이 해시와 규칙 판정에서 제외한다.
고장 임계값은 고정하고 노후도,
...
91%는 임의의 추가 감용이 아니다. 이 대표 운전점에서 노후 저항·허용온도, 기온, 일사와
풍속을 한 번에 받은 `LineThermalModel`이 명판의 83.7%를 현재 허용흐름으로 내므로 명판 기준
76% 흐름은 `0.76 / 0.837 = 90.8%`가 된다. 피크에 회랑 흐름이 14.5% 오르고 같은 모델을
다시 풀면 첫 밸런스 기준 약 104%가 된다. 83.7%는 회귀 스냅샷의 결과 앵커이지
`상태계수×기상계수`를 런타임에서 다시 곱하라는 공식이 아니다. UI는 노후 상태가 저항·
허용온도에 준 영향과 날씨·수요의 영향을 서로 다른 원인 행으로 보여준다.
...
| 장기 캠페인 | 발전기 정비 | 제조사 권고, 진동 경보 | 공급력 감소 | 일정 조정, 대체 발전, 저장 |
...
보상·긴급대응·후속 정비를 나눈다. 마지막에는 시나리오가 미리 허용한 조치 또는 플레이어가
...
시나리오에 미리 등록된 `사전 점검`, 실제 건설 때 제안받았던 `우회선 생략`, 플레이 중
...
4. **결정 시점**: B는 day 36까지 개발 옵션을 사야 day 700 전에 완공할 수 있지만, 10 M만
   먼저 지불하고 day 45 수요계약을 본 뒤 남은 자기자본 110 M의 최종 투자 여부를 정할 수
   있다. A도 옵션을 샀다면 228 M 중 6 M을 선납한 것으로 처리한다.
...
옵션만 지불하고 조사를 진행한 뒤 day 45 계약을 보고 남은 자기자본 110 M의 최종 투자 여부를
결정할 수 있다. 옵션 없이 기다리면 B는 기한을 놓친다. 옵션을 사기 전 후보 카드는 공기
`D-630/D-664`를 소요일수로 보여주고, 옵션 뒤에는 절대 날짜 `COD DAY 700`, 남은 자기자본과
진행률을 보여준다. `옵션 마감`, `최종 투자 마감`, `예상 상업운전일`은 서로 다른 값이다.
...
기본 엔진은 `Godot 4.7.1 .NET`, 언어는 C#으로 고정한다. 저장소의 `global.json`은 검증한
`.NET SDK 8.0.x` 정확한 patch를 pin하고 CI와 개발 PC가 같은 SDK를 쓴다. Godot의 .NET 빌드
요건은 [공식 .NET 컴파일 문서](https://docs.godotengine.org/en/stable/engine_details/development/compiling/compiling_with_dotnet.html)의
SDK 8.0 이상 기준을 따른다. Godot 또는 SDK 버전 변경은 별도 기술결정과 전체 캡처·리플레이
검증을 통과해야 한다. 게임 규칙은 Godot 어셈블리를 참조하지 않는 순수 C#/.NET 라이브러리이며,
...
Gridworks.Core
  ...
  Projects/     공사 DAG, 작업반, 견적, 시운전, 정비와 복구
  Economy/      계량, 계약, 현금·평가 원장, 보상과 프로젝트 금융
...
1.0은 이 다섯 프로젝트만 만든다. 위 하위영역은 폴더·namespace 경계이지 초기부터 각각의
어셈블리로 쪼갤 이유가 아니다. `Core`는 외부 프로젝트를 참조하지 않고,
...
- 선로는 `LineThermalModel`만 기상 입력을 받는다. 전류제곱 발열, 일사, 기온, 풍속과 도체
  상태를 한 열수지에서 계산해 도체온도·정상/비상 허용전류·한계까지 남은 시간을 낸다.
  이 결과에 별도의 `기상×상태` 선로 감용을 다시 곱하지 않는다.
- 변압기는 `TransformerEffectiveRatingResolver`가 매 계산 경계마다 명판정격에서
  `기상×상태×정비`를 한 번만 계산한다. 직전 틱의 유효 정격에 다시 곱하지 않는다.
...
6. [t, t+1분) 고객·발전·연계선 `kW·minute` 계량
...
발전·수입·저장방전과 부하·손실·저장충전·잔차의 정수 합이 정확히 닫혀야 한다. 잔차는 매출,
손실 또는 벌금으로 정산하지 않는다.
...
첫 코드 커밋 전까지 이 문서의 표와 산식이 설계 권위다. 첫 코드 커밋부터 선로, 발전기,
변전소, 계약, 사건과 시나리오 숫자는 `data/catalog/*.json`, `data/scenarios/*.json`과
`data/schema/*.schema.json`이 유일한 기계 권위다. CSV는 긴 수요·기상 시계열에만 사용하고
각 열의 단위·timezone·결측규칙을 동반 schema에 적는다. C# catalog 상수와 Godot scene 내부
복제 숫자는 금지하며, 문서 표·UI 기준값·밸런스 보고서는 기계 권위에서 생성한다. CI는 schema,
단위 누락, 중복 ID, 잘못된 전압 연결, 견적·완공일·화면 표기의 불일치를 차단한다.
...
1. **정적 밸런스 선별**: 기계 권위 데이터와 동일한 산식을 쓰는 비권위 Balance Lab으로
   정상·폭염·단일고장 스냅샷을 검사하고 명백한 불가능 조합과 지배전략을 제거한다.
2. **Headless 재미 검증**: 6~10모선, 발전기 2, 배터리 1, 수요 3, 병원과 두 경로 후보,
   폭염·선로 고장·N-1·비용·보상만 구현한다. 질문은 “비싼 우회선이 사고 뒤 아깝지
   않았는가” 하나다.
3. **2D 공간 건설**: 변전소 배치, 서비스 필드, 자유 경로, 철탑, 지형 비용과 공사 후
   점등을 디버그 도형으로 검증한다.
4. **계통 운영**: 손실 보정 DC 흐름, 자동급전, 열, 저장, 예비력, 자동 부하차단,
   N-1과 단선결선도를 붙인다.
5. **위기와 지속성**: 숨은 상태, 점검·정비, 폭염, 사용불가·복구, 계량·보상,
   저장·리플레이와 사고 보고서를 붙인다.
6. **Godot 3D 제작**: 앞 단계의 재미와 규칙이 고정된 뒤 카메라, 지형, 철탑·전선,
   설비 모듈, UI, 날씨, 사운드와 캡처 자동화를 만든다.
7. **1.0 콘텐츠**: 3장 핵심 캠페인과 고정 챌린지를 제작한다.
8. **장기 확장**: 복합화력·석탄·원전·재생 포트폴리오, 환경제약, 금융과 7장 캠페인을
...
- 소형 가스엔진, 태양광, 배터리. 가스 피커는 3장의 위기 대응용 임대 설비로만 등장
- 66 kV와 154 kV 선로
- 마을·산업·중요시설·1차 변전소
- 임시 우회선
...
수직 슬라이스는 한 번에 전부 만들지 않고 아래 다섯 게이트로 자른다.

1. **정적 타당성 게이트**: Balance Lab에서 단위·현금·용량·공기 산식이 닫히고 서로 다른
   안전 해법이 두 개 이상 남으며 하나의 설계가 모든 스트레스 케이스를 지배하지 않는다.
2. **재미·물리 게이트**: Headless 고정망에서 두 안전해법, 자동급전, 손실·열·N-1과
   정전 보상이 설명 가능한 결과를 만든다.
3. **공간·견적 게이트**: 설비 배치, 서비스 필드, 자유 노선, 전신주 자동 배치, 비용·공기
   비교와 공사 완료까지만 구현한다.
4. **계통·운영 게이트**: 동적 그래프, 전력조류, 용량·손실·열, 발전·저장, 부하 우선순위와
   N-1 미리보기를 붙인다.
5. **위기·지속성 게이트**: 노후도 불확실성, 점검·정비, 폭염, 사용불가·복구, 경제 평가,
   저장과 리플레이를 붙인다.
...
12. 같은 명령을 재생한 화면 표시 운전과 headless 고속 운전의 분 단위 사건·계량·현금·상태
    해시가 정확히 같다.
...
열 차단, 저장 SOC, 정산 단위, 중복 분개·견적 차단과 저장 전후 해시다. 현금·분개·
`EnergyKwMinutes`·사건 분·양자화된 선로 kW는 exact equality로, 양자화 전 도체온도는
`±0.05°C`로 비교한다. `UnsafeSolve`는 이전 틱 흐름을 재사용하지 않고 안전 실패로 닫혀야 한다.
무작위 그래프에서는
전압이 다른 포트 연결, 음수 SOC·현금, 중복 매출, 프리뷰의 권위 상태 변경이 없어야 한다.

기준 개발 PC와 대표 지도·저장본을 저장소에 기록하고 다음 1.0 성능 예산을 회귀 검사한다.
경로 드래그 L0 미리보기 p95는 100 ms 이하, 확정 N-1 분석 p95는 500 ms 이하, 1080p 일반
플레이 프레임 p95는 16.7 ms 이하·draw call 1,500 이하, 120일 캠페인 저장은 25 MB 이하이며
...
`plant-siting`은 장기 캠페인 확장 캡처다. 1.0은 병원의 정상 E1 경로, E1 제거 직후 UPS 유지,
고정 자동절체 뒤 독립 E2 공급을 한 구조 캡처에서 재현해야 한다.
...
가상 정책의 시간을 혼동한다. Gridworks는 아래 세 층을 순서대로 사용한다.

1. **Static Balance Lab**: 정상 피크, 폭염 피크, 단일고장 직후, 15분, 2시간, 복구 종료의
   고정 스냅샷에서 설계 후보를 빠르게 검산한다. 명백한 용량·현금·공기 위반과 지배전략을
   제거하지만 시간순 게임의 정답을 선언하지 않는다.
2. **권위 Headless 실행**: 정적 검사를 통과한 50개 이하 후보만 실제 1분 틱, 열·기동·SOC,
   공사·정산·외생 사건으로 끝까지 재생한다.
3. **사람 플레이테스트**: 재미, 경보 이해, 망설임, 실수 회복, 선택의 후회와 만족은 실제
   플레이로만 승인한다. 정적·자동 정책 점수가 좋아도 사람이 이유를 읽지 못하면 실패다.
...
한 milestone에서 동시에 여는 밸런스 knob는 8~15개로 제한하고 한 A/B 빌드에서는 한
파라미터 계열만 바꾼다. 대규모 탐색은 작은 격자, Latin hypercube 또는 Sobol 표본과 단변수
민감도 분석으로 후보를 줄인다. “점수가 오를 때까지” 무제한 최적화하지 않으며 목표 범위와
실행 예산을 먼저 고정한다. 자동 도구는 최소 두 개의 서로 다른 안전 설계, 가장 싼 위험설계와
가장 싼 안전설계의 비용 간격, 각 스트레스 케이스의 ENS·현금·LCC·복구시간, Pareto 전면과
국소 민감도를 보고한다. 모든 케이스를 한 설계가 압도하면 콘텐츠나 수치가 닫힌 것으로 본다.

사람의 플레이와 가상 정책의 차이는 제거할 잡음이 아니라 별도 데이터다. 동의한 플레이테스트
리플레이에서 `관측한 정보 → 명령 → 반응시간`을 익명 trace로 보존하고, 다음 후보 빌드에서
같은 명령을 deterministic ghost policy로 재생한다. 이 trace는 사람이 보지 못한 미래를
참조하거나 자동으로 명령을 개선하지 않는다. 각 플레이테스트 라운드 뒤에는 문제와 직접
관련된 1~2회의 제한된 sweep만 허용하고, 새 빌드는 변경한 파라미터 계열과 예상 효과를
명시한다. LLM은 가설·경계사례·테스트와 보고서 요약을 제안할 수 있지만 최종 수치 선택,
출시 승인과 사람 행동의 대체물을 맡지 않는다.
