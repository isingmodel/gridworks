...
먼저 검증했다. 직접 배치는 후속 로드맵 단계에서 열지만 완공 자산 철거는 1.0 필수가 아니다.
...
| 변전소 | 전력회사 설비. 현재는 고정 위치에 설치된 상태 | 해당 로드맵 단계에서 직접 배치·발주하고 피더 연결 | 위치·footprint, 건설비·공기, 정격, terminal, 기하학적 서비스 권역 또는 전용부하 | Scope 0 자유 배치·upgrade·철거 |
