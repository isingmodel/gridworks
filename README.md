...
운영하는 싱글 플레이 전력망 전략 게임이다. 통전된 배전 변전소는 주변에 서비스 권역을
만들지만, 그 권역이 발전소까지 실제 경로로 이어져야 전기가 공급된다.
...
Scope 0A가 통과해도 구현은 자동으로 시작되지 않는다. 결과를 검토해 fixture와 다음 가설을
갱신한 뒤, 사용자가 별도로 승인할 때만 10~15분짜리 **Scope 0B 2D playable**을 만든다.
...
구현 승인이 아니다. 문서가 충돌하면 사용자의 현재 명시적 지시, 활성 scope, 게임 기획서,
전문 문서, 콘셉트 이미지 순으로 판단한다.
