...
| 변전소 직접 건설 | `현재 첫 점등 범위`. 한 기의 초안 배치·이동·취소, 별도 발주·원자 완공을 구현 |
| 전신주·철탑을 하나씩 직접 건설 | `현재 첫 점등 범위`. 기존 발전원과 완공 변전소 사이 support 한 종류를 직접 배치 |
| 각 오브젝트 사이 전선 길이 hard limit | `현재 첫 점등 범위`. 인접 endpoint 사이 하나의 `MaxSpan`을 사용 |
...
검증했다. 첫 점등 제품 경로에서 변전소와 선로 직접 건설을 하나의 흐름으로 처음 통합했다. 발전소
신규 건설과 완공 자산 철거는 아직 구현되지 않았다.
...
| 송전·배전선 | 인접 endpoint 사이의 전기 span과 이를 묶은 `LineProject` | authored 회랑 선택; 첫 점등에서는 기존 발전원과 완공 변전소 사이 span 연결·일괄 발주 | 정격, 길이, `MaxSpan`, 비용, 공기, 전기 contingency, 공간 위험 group | 부분 통전·철거·일반 graph 편집 |
| 전신주·철탑 | 전력회사 설비. 첫 점등에서는 제품 선로용 support 한 종류 | 하나씩 위치 지정하고 마지막 support를 되돌리거나 전체 초안을 취소한 뒤 선로를 일괄 발주 | 위치, support class, 건설상태, 소속 `LineProject`, 연결 span과 인접 endpoint까지 거리 | 철거·지지물별 노후·재고·장력 설정 |
...
| 병원 | 현재 활성 단계의 authored 중요 수요처, 전력회사 소유 아님 | 주·예비 회선을 직접 만들고 계통공급·P0 확인 | P0, 서로 다른 terminal·회선, 고객 내부전원, 높은 미공급 책임 | 건설·철거·P0 자동차단·내부전력을 판매로 계량 |
...
현재 첫 점등에서 변전소는 하나의 `AssetProject`, 지지물과 도체로 이루어진 선로는 하나의
`LineProject`다. 플레이어는 변전소를 유효 부지에 놓고, 기존 발전원과 완공 변전소 terminal
사이에 지지물을 하나씩 배치해 선로 경로를 만든다. 발전소 직접 건설이 열리면 같은 최소
...
수요처 하나는 한 경로로 전량 공급되거나 미공급되며 여러 경로에 나누지 않는다. 공유 용량,
경로 동률과 복수 발전원 순서는 각각 처음 필요한 단계 계약이 고정한다.
...
서로 다른 `LineProject`는 지지물과 span을 공유하지 않는다. 정확한 교차 경계값은 병원 신뢰도
단계 계약이 정한다.
