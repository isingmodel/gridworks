...
3. Authority order: current user instruction → active scope → `doc/GAME_DESIGN_KO.md` → specialist docs → images.
...
6. Do not use LLM or policy agents for open-ended tuning or target choice ratios. Keep experiments bounded by
   the parameter policy in `README.md` and `doc/BALANCING_STATIC_SIM.md`.
